...
import com.bw.movie.adapter.more.ComingSoonAdapter;
 ...
 */
public class ComingSoonFragment extends BaseFragment implements IHomeMoreContract.IView {
    @BindView(R.id.xrv)
    XRecyclerView xrv;
    Unbinder unbinder;
    ArrayList<ComingSoonResultBean> list = new ArrayList<>();
    int page = 1;
    int count = 5;
    ...
    protected int getLayout() {
        return R.layout.more_comingsonnefragment;
    }
    ...
    protected void initData() {
        if (RetUtils.getInstance().isNetWork(getContext())){
            ...
            if (presenter instanceof IHomeMorePresenter){
                ((IHomeMorePresenter) presenter).getComingSoon(page,count);
    ...
    public void onComingSoonSuccess(ComingSoonBean comingSoonBean) {
        ...
        ComingSoonAdapter moreComingSoonAdapter = new ComingSoonAdapter(getContext(), list);
        xrv.setAdapter(moreComingSoonAdapter);
    }
